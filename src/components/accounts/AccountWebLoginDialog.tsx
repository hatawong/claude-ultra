/**
 * AccountWebLoginDialog — Web login dialog.
 */
...
interface AccountWebLoginDialogProps {
  account: Account;
  onClose: () => void;
  onDone: () => void;
}

export default function AccountWebLoginDialog({ account, onClose, onDone }: AccountWebLoginDialogProps) {
  const { t } = useTranslation();
  const taskId = `web-login-${account.accountId}`;
  ...
  const task = useTaskDialog({
    ...
    onEvent: (type, data) => {
      ...
      if (type === 'cookies' && data.sessionKey) {
        invoke('update_web_login', {
          accountId: account.accountId,
          cookies: data.cookies || [],
          sessionKey: data.sessionKey,
          proxy: null,
        }).catch((e: any) => console.error('Failed to save cookies:', e));
      }
      if (type === 'result' && data.success) {
        ...
        }).catch((e: any) => console.error('Failed to update web login:', e));
        // Update profile info (email etc. may be empty at add time)
        invoke('update_account_profile', {
          accountId: account.accountId,
  ...
  const handleStart = useCallback(async () => {
    ...
    try {
      await invoke<string>('start_web_login', { accountId: account.accountId });
    } catch (e: any) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 pt-5 pb-3">
          <div>
            <div className="text-sm font-semibold text-base-content">{t('task.webLogin', 'Web Login')}</div>
            <div className="flex items-center gap-1.5 mt-1">
              {account.email && <span className="text-xs text-gray-400">{account.email}</span>}
              {(account as any).loginMethod && (
                <span className="px-1.5 py-0 rounded text-[10px] font-semibold bg-blue-500/20 text-blue-400">
                  {(account as any).loginMethod}
                </span>
              )}
            </div>
          ...
          <div className="px-6 py-3 border-t border-base-200">
            <div className="flex items-center gap-3 mb-2">
              <span className="text-xs text-gray-400">
                Step {task.step}/{task.totalSteps} · {task.stepName}
              </span>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                {task.error && (
                  <span className="text-xs text-red-500 max-w-[400px] whitespace-pre-line" title={task.error}>: {task.error}</span>
                )}
              </div>
              <div className="flex items-center gap-1.5">
                {(task.status === 'running' || task.status === 'paused') && (
                  <>
                    ...
                    <button
                      onClick={task.handleAbort}
                      className="px-2.5 py-1 text-xs font-medium bg-red-900/20 text-red-400 rounded-lg hover:bg-red-900/30 transition-colors flex items-center gap-1"
                    >
          ...
          <div className="bg-gray-900 rounded-lg p-3 h-full overflow-y-auto font-mono text-xs text-gray-300 leading-relaxed">
            {task.logContent ? (
              ...
              <span className="text-gray-500">
                {task.status === 'idle' ? t('task.clickToStart', 'Click "Start Login" to begin') : t('task.waitingLog', 'Waiting for logs...')}
              </span>
            )}
          ...
          <div className="flex items-center gap-2">
            {task.status === 'idle' && (
              <button onClick={handleStart} disabled={!preflight.ready} className={cn("px-4 py-1.5 text-xs font-medium rounded-lg transition-colors flex items-center gap-1", preflight.ready ? "bg-blue-500 text-white hover:bg-blue-600" : "bg-gray-600 text-gray-400 cursor-not-allowed")}>
                ...
                {t('task.startLogin', 'Start Login')}
              </button>
            )}
            {(task.status === 'done' || task.status === 'failed') && (
              <button onClick={handleStart} disabled={!preflight.ready} className={cn("px-4 py-1.5 text-xs font-medium rounded-lg transition-colors flex items-center gap-1", preflight.ready ? "bg-blue-500 text-white hover:bg-blue-600" : "bg-gray-600 text-gray-400 cursor-not-allowed")}>
                <Play className="w-3 h-3" />
                {t('task.retryLogin', 'Retry Login')}
              </button>
            )}
            {task.status !== 'running' && task.status !== 'paused' && (
